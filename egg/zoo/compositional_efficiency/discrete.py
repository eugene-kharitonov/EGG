...
from egg.zoo.compositional_efficiency.archs import (Receiver, IdentitySender, RotatedSender)

from egg.zoo.compositional_efficiency.intervention import Evaluator, Metrics
...
def get_params(params):
    ...
    parser.add_argument('--loss_type', choices=['autoenc', 'mixed', 'linear'], default='autoenc')
...
class DiffLoss(torch.nn.Module):
    ...
    def forward(self, sender_input, _message, _receiver_input, receiver_output, _labels):
        ...
        if self.loss_type == 'mixed':
            ...
            right = (sender_input[:, 0] - sender_input[:, 1] + self.n_values).fmod(self.n_values)
            sender_input[:, 0], sender_input[:, 1] = left, right
        elif self.loss_type == 'linear':
            a, b, c, d, e, f = self.coeffs
            left = (a * sender_input[:, 0] + b * sender_input[:, 1] + c).fmod(self.n_values)
            right = (d * sender_input[:, 0] + e * sender_input[:, 1] + f).fmod(self.n_values)
            sender_input[:, 0], sender_input[:, 1] = left, right
...
def main(params):
    ...
    if opts.receiver_cell == 'transformer':
            ...
            receiver = core.TransformerReceiverDeterministic(receiver, 
                opts.vocab_size + 1, 5, opts.receiver_emb, num_heads=2, hidden_size=opts.receiver_hidden, num_layers=opts.cell_layers,
                causal=False)
    else:
    ...
    if False:
        for k, _ in train_data:
            ...
            message = sender(k)[0]
            receiver_out = receiver(message)[0]

            print(k, message, receiver_out.view(2, -1).argmax(dim=-1))
