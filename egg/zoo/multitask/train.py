...
def get_params(params):
    ...
    parser.add_argument('--task_embed_dim', type=int, default=10)
    ...
    parser.add_argument('--task_embedding', default=False, action='store_true',
                        help='if set, Sender will receive a task embedding')
    parser.add_argument('--dump_message_folder', default=None,
                        help='folder where messages for the test set will be stored')
    parser.add_argument('--output', default='/private/home/rdessi/logging_stats',
                        help='folder where metrics will be stored')
    parser.add_argument('--store_language', default=False, action='store_true',
                        help='if set, languages generated by the agents will be saved')
...
def main(params):
    ...
    if opts.task_embedding:
        sender = SenderTaskEmbedding(n_hidden_features=opts.sender_hidden,
                        n_features=features,
                        n_features_task=num_tasks,
                        n_hidden_task=opts.task_embed_dim)
        sender_embed_dim = opts.sender_hidden+opts.task_embed_dim
    else:
        sender = Sender(n_hidden_features=opts.sender_hidden, n_features=features)
        sender_embed_dim = opts.sender_hidden
    ...
    games = [core.SenderReceiverRnnReinforce(sender, receiver, loss, sender_entropy_coeff=opts.sender_entropy_coeff,
                                           receiver_entropy_coeff=opts.receiver_entropy_coeff) for (receiver, loss) in zip(receivers, losses)]

    optimizers = [core.build_optimizer(game.parameters()) for game in games]

    output_file = get_output_file(opts)
    #output_topsim = pathlib.Path(opts.output) / f'language_seed_{opts.random_seed}'
    #output_topsim.mkdir()

    opts.store_language = True
    assert opts.store_language
    callbacks = [core.ConsoleLogger(as_json=True, print_train_loss=True, output_file=output_file),
                 ...
                 #EvaluateTopSim(opts.n_attributes,
                 #               opts.n_values,
                 #               val_it,
                 #               'val_set',
                 #               games[0].sender,
    ...
    trainer = core.Trainer(games=games, optimizers=optimizers, train_data=train_it, validation_data=None, callbacks=callbacks)
