...
from egg.zoo.population_compo_generalization.data import ScaledDataset, enumerate_attribute_value, split_train_test, one_hotify, split_holdout
from egg.zoo.population_compo_generalization.archs import Sender, Receiver, \
    Freezer, PlusOneWrapper, NonLinearReceiver
from egg.zoo.population_compo_generalization.intervention import Evaluator

from egg.core import EarlyStopperAccuracy
from egg.core.callbacks import TopographicSimilarity
from egg.core.population import UniformAgentSampler, PopulationGame, FullSweepAgentSampler
...
def get_params(params):
    ...
    parser.add_argument('--n_senders', type=int, default=1, help='')
    parser.add_argument('--n_receivers', type=int, default=1, help='')
...
def main(params):
    ...
    assert opts.receiver_cell in ['lstm', 'rnn', 'gru']
    assert opts.sender_cell in ['lstm', 'rnn', 'gru']
    ...
    senders = [get_sender() for _ in range(opts.n_senders)]
    receivers = [get_receiver() for _ in range(opts.n_receivers)]
    ...
    topographic = TopographicSimilarity('hamming')
    ...
    # Train new agents
    if False and validation_acc > 0.99:
        def _set_seed(seed):
            import random
            import numpy as np
