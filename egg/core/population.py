...
class FullSweepAgentSampler(nn.Module):
    ...
    def reset_order(self):
        ...
        self.iterator = itertools.product(self.senders_order, self.receivers_order, self.losses_order)
...
class FullSweepAgentSequentialSampler(nn.Module):
    ...
    def forward(self):
        ...
        if len(self.senders) > 1:
            s, r, l = self.senders[self.idx], self.receivers[self.idx], self.losses[self.idx]
        else:
            s, r, l = self.senders[0], self.receivers[self.idx], self.losses[self.idx]
