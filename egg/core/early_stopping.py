...
class EarlyStopperAccuracy(BaseEarlyStopper):
    ...
    is achieved.
    """
    def __init__(self, threshold: float, field_name: str = 'acc', validation=True) -> None:
        """
        ...
        :param validation: whether the statistics on the validation (or training) data should be checked
        """
    ...
    def should_stop(self) -> bool:
        if self.validation:
            assert self.validation_stats is not None, 'Validation data must be provided for early stooping to work'
            stats = self.validation_stats
        else:
            assert self.train_stats is not None, 'Training data must be provided for early stooping to work'
